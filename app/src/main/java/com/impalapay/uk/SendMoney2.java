...
@SuppressLint("NewApi")
public class SendMoney2 extends FragmentActivity implements TextWatcher,
		OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.send_money2);

		init();
		Common_data.setupUI(findViewById(R.id.hide), SendMoney2.this);
		
		dummy = new DummyAdapter(getApplicationContext(), 0, array_clone);

		parent_scroll.setOnTouchListener(new OnTouchListener() {
		...
		scroll1.setOnTouchListener(new OnTouchListener() {
		...
		scroll2.setOnTouchListener(new OnTouchListener() {
	...
	private void init() {
		...
		country_code_sp.setOnItemSelectedListener(new OnItemSelectedListener() {
				...
				if (validation()) {
					...
					if(SendMoney1_Frag.type.equals("ewallet"))
					{
						...
						{*/
							if(getValidSafariMobile(mobile_static.replace(" ","").replace("-", "").replace("(","").replace(")","").trim())){
								...
								if(1 == is_contact_from_phonebook){
									...
									//}
									Intent i = new Intent(SendMoney2.this, SendMoney3.class);
								...
								Toast.makeText(SendMoney2.this, "Please Provide a valid Safaricom mobile number", Toast.LENGTH_SHORT).show();
							}
						/*}
						else{
						...
						if(getValidUgAirtelMSISSDN(mobile_static.replace(" ", "").replace("-", "").replace("(", "").replace(")", "").trim())){
							...
							Toast.makeText(SendMoney2.this, "Please Provide a valid Ugandan Airtel mobile number.", Toast.LENGTH_SHORT).show();
						}
						/*}
						else{
						...
						if(getValidUgMTNMSISSDN(mobile_static.replace(" ", "").replace("-", "").replace("(", "").replace(")", "").trim())){
							...
							Toast.makeText(SendMoney2.this, "Please Provide a valid Ugandan MTN mobile number.", Toast.LENGTH_SHORT).show();
						}
						/*}
						else{
						...
						if(getValidZimTelecashMSISSDN(mobile_static.replace(" ", "").replace("-", "").replace("(", "").replace(")", "").trim())){
							...
							Toast.makeText(SendMoney2.this, "Please Provide a valid Zimbabwe Telecash mobile number.", Toast.LENGTH_SHORT).show();
						}
						/*}
						else{
						...
						if(account_no.length() <= 0){
								Toast.makeText(SendMoney2.this, "Recipient's bank account number should be available. Please Add Recipient first.", Toast.LENGTH_SHORT).show();
		...
		add_recipent.setOnClickListener(new OnClickListener() {
			...
			public void afterTextChanged(Editable s) {
				...
				if (allList.get(0).size() != 0) {
					...
					for (int i = 0; i < allList.get(0).size(); i++) {
						...
						beni.setOnClickListener(new OnClickListener() {

							@Override
							public void onClick(View v) {

								int position = (Integer) v.getTag();
								MainActivity.hideSoftKeyboard(SendMoney2.this, v);
								...
								Toast.makeText(SendMoney2.this, "" + allList.get(0).get(position).getName() + " is Selected", Toast.LENGTH_SHORT).show();
				...
				if (allList.get(1).size() != 0) {
					...
					for (int i = 0; i < allList.get(1).size(); i++) {
						...
						aBook.setOnClickListener(new OnClickListener() {
							...
							public void onClick(View v) {
								...
								if(split_name.length < 2){
									final Dialog d=new Dialog(SendMoney2.this);
								...
								Toast.makeText(SendMoney2.this, "" + allList.get(1).get(position).getName() + " is Selected", Toast.LENGTH_SHORT).show();
		...
		ArrayAdapter<String> countryAdapter = new ArrayAdapter<String>(SendMoney2.this,R.layout.custom_spinner_text, country_al);
	...
	public void addRecipient(){
		...
		if (flag) {
			 ...
			 TextView code_warn = (TextView) findViewById(R.id.code_warning);
			 if (warn_code){
				 f_name_beni_et.setText(c_name);
				 if (c_contact.length() > 9){
					 confirm_number_beni_et.setText(c_contact.substring(c_contact.length() - 9));
					 number_beni_et.setText(c_contact.substring(c_contact.length() - 9));
				 }else {
					 confirm_number_beni_et.setText(c_contact);
					 number_beni_et.setText(c_contact);
				 }
				 code_warn.setVisibility(View.VISIBLE);
				 if (SendMoney1_Frag.destination.equalsIgnoreCase("Kenya")){
					 country_code_sp.setSelection(0);
					 confirm_country_code_sp.setSelection(0);
				 } else if (SendMoney1_Frag.destination.equalsIgnoreCase("Uganda")){
					 country_code_sp.setSelection(1);
					 confirm_country_code_sp.setSelection(1);
				 } else if (SendMoney1_Frag.destination.equalsIgnoreCase("Zimbabwe")){
					 country_code_sp.setSelection(2);
					 confirm_country_code_sp.setSelection(2);
				 }
			 }else {
			 	code_warn.setVisibility(View.GONE);
			 }
			 confirm_country_code_sp.setOnItemSelectedListener(new OnItemSelectedListener() {
				 @Override
				 public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
					 country_code_sp.setSelection(i);
				 }
			 ...
			 country_code_sp.setOnItemSelectedListener(new OnItemSelectedListener() {
				 @Override
				 public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
					 confirm_country_code_sp.setSelection(i);
				 }
	...
	class BankNameHandler extends AsyncHttpResponseHandler{
		...
		public void onStart() {
			super.onStart();
			pdialog=ProgressDialog.show(SendMoney2.this, "", "Loading...");
		}
					...
					if(b)
					{
						...
						btn_ok.setOnClickListener(new View.OnClickListener() {
							...
							public void onClick(View arg0) {
								...
								Intent i = new Intent(SendMoney2.this, Login.class);
								startActivity(i);
							}
						});
						d.show();
		...
		public void onFailure(Throwable error) {
			...
			networkErrorDialog(SendMoney2.this);
		}
	}
...
private void getBranchName(String bank) {
	...
	class BranchNameHandler extends AsyncHttpResponseHandler{
		...
		public void onStart() {
			super.onStart();
			pdialog=ProgressDialog.show(SendMoney2.this, "", "Getting Bank Branches...");
		}
	...
	private void showBeneficiary() {
		...
		try {
			...
			//object.put("userid", Common_data.getPreferences(SendMoney2.this, "userid"));
	...
	class showBeneficaryHandler extends AsyncHttpResponseHandler {
		...
		public void onStart() {
			super.onStart();
			pdialog = ProgressDialog.show(SendMoney2.this, "", "Retrieving beneficiaries...");
		}
			...
			if (content.length() > 0) {
				...
				try {
					...
					if (b) {
						...
						Intent i = new Intent(SendMoney2.this, MainActivity.class);
						startActivity(i);
					}
	...
	private boolean validationBank() {
		...
		if(f_name.length()< 3)
		{
			...
			Toast.makeText(SendMoney2.this, "First Name should have at least 3 characters", Toast.LENGTH_SHORT).show();
		}
        else if(l_name.length()< 3)
        {

            Toast.makeText(SendMoney2.this, "Last Name should have at least 3 characters", Toast.LENGTH_SHORT).show();
        }
		else if(! country_code.equals(confirm_country_code)){
			Toast.makeText(SendMoney2.this, "Country code does not match with its confirmation", Toast.LENGTH_SHORT).show();
		}
		else if(mobile.length()==0){
			Toast.makeText(SendMoney2.this, "Enter mobile number", Toast.LENGTH_SHORT).show();
		}
		else if(! mobile.equals(confirm_mobile)){
			Toast.makeText(SendMoney2.this, "Mobile number does not match with its confirmation", Toast.LENGTH_SHORT).show();
		}
		else if(mobile.length() != 9 && country_code.equals("+254")){
			Toast.makeText(SendMoney2.this, "Mobile number should have 9 digits", Toast.LENGTH_SHORT).show();
		}
		else if(mobile.length() != 10 && country_code.equals("+1")){
			Toast.makeText(SendMoney2.this, "Mobile number should have 10 digits", Toast.LENGTH_SHORT).show();
		}
		else if(email.length() > 0)
		{
			if(!email.matches(Constant.emailPattern)){
				Toast.makeText(SendMoney2.this, "Enter valid recipient's email", Toast.LENGTH_SHORT).show();
			}
		}
		else if(city.length()==0)
		{
			Toast.makeText(SendMoney2.this, "City/Town cannot be blank", Toast.LENGTH_SHORT).show();
		...
		else if(country.equals("---Select---"))
		{
			//Toast.makeText(SendMoney2.this, "Please select country", Toast.LENGTH_SHORT).show();
			 ...
			 account_no=ac_no;
			if(ac_na.length()==0)
				Toast.makeText(SendMoney2.this, "Enter name as per bank record", Toast.LENGTH_SHORT).show();
			else if(ac_no.length()==0)
				Toast.makeText(SendMoney2.this, "Enter account number", Toast.LENGTH_SHORT).show();
			else if(! ac_no.equals(confirm_ac_no))
				Toast.makeText(SendMoney2.this, "Account number does not match with its confirmation", Toast.LENGTH_SHORT).show();
			else if(bankname.equals("---Select---"))
			{
				Toast.makeText(SendMoney2.this, "Please select bank name", Toast.LENGTH_SHORT).show();

			}
			else if( (branchname.equals("---Select---")) || (branchname.length()==0) || (branchname.equals("")) )
			{
				Toast.makeText(SendMoney2.this, "Please select branch name", Toast.LENGTH_SHORT).show();
	...
	class SaveBeneficaryHandler extends AsyncHttpResponseHandler{
		...
		public void onStart() {
			super.onStart();
			pdialog=ProgressDialog.show(SendMoney2.this, "", "Saving...");
		}
			...
			if(content.length()>0){
				...
				try {
					...
					if(b)
					{
						...
						Toast.makeText(SendMoney2.this, "Beneficiary added successfully", Toast.LENGTH_SHORT).show();
						add_recp_ly.setVisibility(View.GONE);
						select_rec_tv.setVisibility(View.VISIBLE);
						edit.setVisibility(View.VISIBLE);
						...
						Toast.makeText(SendMoney2.this, msg, Toast.LENGTH_SHORT).show();
					}
					...
					if (content.length() > 0) {
						...
						try {
							...
							if (b) {
								...
								else if(msg.equalsIgnoreCase("This User is not Registered with SawaPay")){
									name_static="";
									mobile_static="";
									email_static="";
									Toast.makeText(getApplicationContext(), msg, Toast.LENGTH_LONG).show();
	...
	public void showUpdateDialog(){
		...
		dialog=new Dialog(SendMoney2.this);
		...
		cancel_beni.setOnClickListener(new OnClickListener() {
		...
		choose_bank_sp_beni.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onClick(View v) {
				try {
					...
					if(choose_bank_sp_beni.getSelectedItem().toString().length()==0){
						Toast.makeText(SendMoney2.this, "select Bank Name", Toast.LENGTH_SHORT).show();
					}
					else if(choose_bank_branch_sp_beni.getSelectedItem().toString().length()==0){
						Toast.makeText(SendMoney2.this, "select branch Name", Toast.LENGTH_SHORT).show();
					}
					else if(named.getText().toString().trim().length()==0){
						Toast.makeText(SendMoney2.this, "Enter Name", Toast.LENGTH_SHORT).show();
					}
